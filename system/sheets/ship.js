  ...
  activateListeners(html) {
    if (this.isEditable) {
      if (this.actor.isOwner) {
        html.querySelectorAll('.track').forEach(el => el.addEventListener('click', this.adjustTrack.bind(this, 1)));
        html.querySelectorAll('.track').forEach(el => el.addEventListener('contextmenu', this.adjustTrack.bind(this, -1)));

        const addItemBtn = html.querySelector('.addItem'); if (addItemBtn) addItemBtn.addEventListener('click', this.addItem.bind(this));
        const dmgBtn = html.querySelector('.damage-button'); if (dmgBtn) dmgBtn.addEventListener('click', this.toggleFittingDamaged.bind(this));
      }
      const ratingBtn = html.querySelector('.ratingRoll'); if (ratingBtn) ratingBtn.addEventListener('click', this.ratingRoll.bind(this));
    }
