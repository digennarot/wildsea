...
export class WildseaTrackPanel extends Application {
  ...
  activateListeners(html) {
    if (game.user.isGM) {
      html.querySelector('.addTrack')?.addEventListener('click', this.addTrack.bind(this));
      html.querySelector('.label')?.addEventListener('click', this.editTrack.bind(this));
      html.querySelectorAll('.delete').forEach(el => el.addEventListener('click', this.interactWithTrack.bind(this, 'delete')));
      html.querySelectorAll('.slots').forEach(el => el.addEventListener('click', this.interactWithTrack.bind(this, 'mark')));
      html.querySelectorAll('.slots').forEach(el => el.addEventListener('contextmenu', this.interactWithTrack.bind(this, 'unmark')));

      const trackList = html.querySelector('.track-list');
      new SortableJS(trackList, {
        animation: 200,
        direction: 'vertical',
        draggable: '.track',
